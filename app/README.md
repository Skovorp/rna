...
- **Compare conditions:** compare every gene between two conditions in one study; view an MA plot of average TPM versus exact log₂(A/B) with FDR-driven opacity, plus TPM summaries, raw Welch p-values, and Benjamini–Hochberg FDR. Genes with zero mean TPM in either condition remain in the table but are omitted from the plot because their log ratio is undefined.
