...
def _segmented_controls(app):
    return (
        app.segmented_control
        if hasattr(app, "segmented_control")
        else app.get("button_group")
    )
...
@pytest.mark.parametrize(
    ...
    [
        ("2_Ovary_paper_vs_reprocessed.py", "Ovary: published vs reprocessed", 33),
        ("4_Atlas_paper_vs_reprocessed.py", "Neurotranscriptome: published vs reprocessed", 122),
    ],
)
def test_comparison_page_renders_native_figures(
    ...
    assert [control.value for control in _segmented_controls(app)] == ["TPM agreement"]
    assert any(
    ...
    for analysis, heading, table_count in (
        ...
        _set_segmented_value(_segmented_controls(app)[0], analysis).run()
        assert not app.exception, [exception.message for exception in app.exception]
        assert len(app.get("plotly_chart")) == 1
