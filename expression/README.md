...
- `midgut_deseq2/`: all 28 pairwise DESeq2 result tables across the eight midgut conditions, generated by `nf-core/differentialabundance` 2.0.0 from Salmon gene-level counts.
- `elife_deseq2/`: all 55 pairwise DESeq2 result tables across the eleven ovary conditions, generated by `nf-core/differentialabundance` 2.0.0 from the recomputed Salmon gene-level counts for BioProject `PRJNA796320`.
...
Each differential directory contains `contrasts.tsv`, the compressed pipeline tables, and `provenance.json` with source and bundled checksums. The result columns are the pipeline's `baseMean`, `log2FoldChange`, `lfcSE`, `pvalue`, and `padj`; no differential statistics are computed in the app.
